define([
...
], function (SuperClass, BreaseEvent, Scroller, Enum, Utils, Types, dragAndDropCapability, ContentActivatedDependency, BreaseResizeObserver) {
    ...
    p.init = function () {
        ...
        var self = this;
        this.wrapperId = this.elem.id + '_scrollWrapper';
        this.observer = new BreaseResizeObserver(this.elem, this._bind('_refreshScroller'));
        ...
        $('#' + this.wrapperId).children().each(function () {
    ...
    p.dispose = function () {
        window.clearTimeout(this.timeoutId);
        this.buttonReady.forEach(function (readyDeferred) {
    ...
    function _setAlignmentClass(widget) {
        ...
        widget.container.off(BreaseEvent.VISIBLE_CHANGED, widget._bind('_scrollUpdateHandler'));
        ...
        if (widget.settings.childPositioning === Enum.ChildPositioning.relative) {
            ...
            if (!widget.scrollWrapper && widget.settings.autoScroll) {
                ...
                widget.container.on(BreaseEvent.VISIBLE_CHANGED, widget._bind('_scrollUpdateHandler'));
    ...
    function _addScroller(widget, scrollerSettings) {
        ...
        widget.scrollWrapper = $('<div/>').addClass('scrollWrapper').attr('id', widget.wrapperId);
    ...
    function _removeScroller(widget) {
        var children = widget.container.children('#' + widget.wrapperId).children();
